...
BATCH_SIZE    = 128        # 批大小 (减小以降低内存使用)
LR            = 1e-3       # 学习率
EPOCHS        = 50         # 训练总轮次
PATIENCE      = 5          # 早停耐心值
TRAIN_VAL_SPLIT = 0.8      # 训练集比例
NUM_WORKERS   = 4          # 数据加载器工作线程数 (根据CPU调整)
DEVICE        = torch.device("cuda" if torch.cuda.is_available() else "cpu")
CHECKPOINT_DIR = "./outputs/checkpoints/improved_train_gnn_vr"
LOG_INTERVAL  = 10         # 每N批次打印一次信息
DEBUG_MODE    = False      # 调试模式 (使用较小的数据集)
...
def evaluate(model, data_loader, criterion, device, max_batches=None):
    ...
    # 添加进度条以显示验证进度
    with torch.no_grad(), tqdm(total=len(data_loader), desc="验证中") as pbar:
...
def train():
    ...
    if DEBUG_MODE:
        # 使用部分数据集进行快速测试
        subset_size_train = min(50000, len(train_dataset))
        subset_size_val = min(10000, len(val_dataset))
        
        # 创建数据子集
        from torch.utils.data import Subset
        
        train_indices = random.sample(range(len(train_dataset)), subset_size_train)
        val_indices = random.sample(range(len(val_dataset)), subset_size_val)
        
        train_dataset = Subset(train_dataset, train_indices)
        val_dataset = Subset(val_dataset, val_indices)
        
        print(f"调试模式: 使用数据子集 训练={len(train_dataset)}窗口, 验证={len(val_dataset)}窗口")
    ...
    train_loader = DataLoader(
        ...
        prefetch_factor=2 if NUM_WORKERS > 0 else None         # 预加载批次
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # 验证阶段
        # 对于非常大的验证集，可以限制验证批次数量以加快每轮验证速度
        # 前10轮使用小批量验证，之后每5轮做一次完整验证
        if epoch <= 10 or epoch % 5 == 0:
            max_val_batches = None  # 完整验证
            print("执行完整验证...")
        else:
            max_val_batches = 100  # 限制为100个批次的快速验证
            print(f"执行快速验证 (限制{max_val_batches}批)...")
            
        val_loss, val_r2, val_mae = evaluate(model, val_loader, criterion, DEVICE, max_batches=max_val_batches)
